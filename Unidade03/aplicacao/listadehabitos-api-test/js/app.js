...
app.controller("apitestgetcontroller", ["$scope", "$http", function ($scope, $http) {
    ...
    $scope.listaDeHabitosApiGet = function () {
        if ($scope.id === "") {
            ...
            $http.get("http://localhost/programacaoweb1/unidade03/coesaoCoplamento/listadehabitos-rest-api/habito.php", {
                params: {
                    id: $scope.id
                }
            }).then(function (data) {
...
app.controller("apitestputcontroller", ["$scope", "$http", function($scope, $http) {
    ...
    $scope.listaDeHabitosApiPut = (function () {
        $http.put("http://localhost/programacaoweb1/unidade03/coesaoCoplamento/listadehabitos-rest-api/habito.php", {
            id: $scope.id,
            nome: $scope.nome,
            status: $scope.status
        }).then(function (data) {
...
app.config(function ($routeProvider) {
    $routeProvider.when("/api-test-get", {
        controller: "apitestgetcontroller", templateUrl: "partials/api-test-get.html"
    }).when("/api-test-post", {
        controller: "apitestpostcontroller", templateUrl: "partials/api-test-post.html"
    }).when("/api-test-put", {
        controller: "apitestputcontroller", templateUrl: "partials/api-test-put.html"
    }).when("/api-test-delete", {
        controller: "apitestdeletecontroller", templateUrl: "partials/api-test-delete.html"
    }).otherwise({
        controller: "welcomecontroller", templateUrl: "partials/welcome.html"
    });
});